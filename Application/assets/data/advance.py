...
x = list(range(int(sys.argv[4]), int(sys.argv[5])))
y = []
for i in x:
    labels = cluster.MiniBatchKMeans(n_clusters = i, random_state=42).fit_predict(reshapedSignal)
    y.append(metrics.davies_bouldin_score(reshapedSignal, labels))
    del labels
    
count = np.argmin(y) + x[0]

labels = cluster.MiniBatchKMeans(n_clusters = count, random_state=42).fit_predict(reshapedSignal)
